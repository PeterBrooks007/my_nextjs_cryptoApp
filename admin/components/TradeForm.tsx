...
import { useAddTrade } from "@/hooks/useAdminOperators";
...
const formSchema = z.object({
  ...
  profitOrLossAmount: z.coerce
    ...
    .min(1, "profitOrLossAmount is required"),
...
const TradeForm = ({
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    const tradeBalance = singleUser?.balance;

    if (type === "Live" && data?.amount > tradeBalance) {
      return toast.error("Insufficient trade balance to trade with");
    }

    const demoBalance = singleUser?.demoBalance;

    if (type === "Demo" && data?.amount > demoBalance) {
      ...
      <FieldGroup className="gap-3 ">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                <FieldLabel htmlFor={field.name}>Long or Short Unit</FieldLabel>
                <InputGroup>
                  <InputGroupInput
                    {...field}
                    id={field.name}
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <Field data-invalid={fieldState.invalid}>
              <FieldLabel htmlFor={field.name}>Amount Win or Lose</FieldLabel>
              <InputGroup>
                <InputGroupInput
                  {...field}
                  id={field.name}
