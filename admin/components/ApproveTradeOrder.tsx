...
const formSchema = z.object({
  ...
  profitOrLossAmount: z.coerce
    ...
    .min(0.0001, "profitOrLossAmount is required"),
...
const ApproveTradeOrder = ({
  ...
  const { adminApproveUserTrade, isAdminApprovingUserTrade } =
    useTradeHistory(id);
  ...
  async function onSubmit(data: FormSchema) {
    const formData = {
      ...
      tradeData: {
        ...
        ...data,
        profitOrLossAmount:
          data.status === "Lose" ? data?.amount : data?.profitOrLossAmount ?? 0,
      },
    };

    // console.log(formData);
    await adminApproveUserTrade(id, formData);
  }

  if (pageLoading || isAdminApprovingUserTrade) {
    return (
