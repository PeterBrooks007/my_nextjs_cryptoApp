...
export function CountdownTimer({
  ...
  const handleTradeProcess = useCallback(() => {
    ...
    if (trades?.tradeFrom === "admin" && trades?.isProcessed === false) {
      ...
      autoTradeUpdateInAdmin(userId, formData);
    }
  }, [user, id, trades, autoTradeUpdateInAdmin]);
...
const TradeHistoryOrdersComp = ({
  ...
  const {
    autoTradeUpdateInAdmin,
    isAutoTradeUpdateInAdminLoading,
    cancelTradeHistory,
  ...
  return (
    <>
                ...
                <div className="flex justify-between">
                  ...
                  {!expiredTrades[trade?._id] ? (
                    <CountdownTimer
                      ...
                      trades={trade}
                      autoTradeUpdateInAdmin={autoTradeUpdateInAdmin}
                    />
                  ) : (
                    <span
                      className={cn(
      ...
      >
        <DialogContent className="sm:max-w-[450px] h-[450px] sm:h-[500px] max-h-[90%] overflow-auto p-0 bg-secondary">
          <VisuallyHidden>
