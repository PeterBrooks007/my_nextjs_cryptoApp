...
type UpdateTradingSignalProps = {
  selectedSignal: TradingBotType | null;
};
...
export const formSchema = z.object({
  ...
  dailyTrades: z.coerce
    .number({
      error: "Must be a number.",
    })
    .min(1, "Daily must be minimum 1.")
    .max(1000, "Daily Trades cannot exceed 1000 trades"),
  winRate: sanitizedString("Win rate contains invalid characters")
    .trim()
    .regex(
      /^\d+%?$/,
      'Win rate must contain only numbers and optionally a "%" symbol'
    )
...
const UpdateTradingSignal = ({ selectedSignal }: UpdateTradingSignalProps) => {
  ...
  const {
    updateTradingSignal,
    isUpdatingTradingSignal,
    updateTradingSignalPhoto,
    isUpdatingTradingSignalPhoto,
  } = useTradingSignals();
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      dailyTrades: selectedSignal?.dailyTrades ?? 0,
      winRate: selectedSignal?.winRate ?? "",
      comment: selectedSignal?.comment ?? "",
    },
    mode: "onChange",
  });
  // updateExpert Trader
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    const id = selectedSignal?._id;

    await updateTradingSignal(id, userData);

    // form.reset(); // clear input
  }

  // update Trading Signal Photo
  const savePhoto = () => {
    if (!profileImage) {
    ...
    updateTradingSignalPhoto(id, formData);
  };
    ...
    <form id="form-rhf-demo" onSubmit={form.handleSubmit(onSubmit)}>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use this form to update this trading Signal details
          </CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-28 min-w-28 h-28">
              <Image
                src={
                  imagePreview ??
                  (selectedSignal?.photo && selectedSignal.photo !== "No Photo"
            ...
            <div>
              <h3 className="text-lg font-semibold">{selectedSignal?.name}</h3>
              <p className="text-sm text-muted-foreground">
                Click the camera icon to change the Signal Icon
              </p>
            </div>
          </div>
          ...
          {imagePreview && (
            <div className="w-full space-x-2 mb-3 flex items-center">
              <Button
                ...
                size="sm"
                onClick={savePhoto}
                disabled={isUpdatingTradingSignalPhoto}
          ...
          <FieldGroup className="gap-5 ">
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor={field.name}>Signal Name</FieldLabel>
                    <InputGroup>
                      <InputGroupInput
                        {...field}
                        id={field.name}
            ...
            <div className="grid grid-cols-2 gap-2">
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor={field.name}>Daily Trades</FieldLabel>
                    <InputGroup>
                      <InputGroupInput
                        {...field}
                        id={field.name}
        ...
        <CardFooter>
          <Button
            disabled={isUpdatingTradingSignal}
            className="w-full"
            type="submit"
          >
            {isUpdatingTradingSignal && <Spinner />}
...
export default UpdateTradingSignal;
