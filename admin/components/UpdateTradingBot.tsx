...
type UpdateTradingBotProps = {
  selectedBot: TradingBotType | null;
};
...
const UpdateTradingBot = ({ selectedBot }: UpdateTradingBotProps) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      price: selectedBot?.price ?? 0,
      dailyTrades: selectedBot?.dailyTrades ?? 0,
      winRate: selectedBot?.winRate ?? "",
      comment: selectedBot?.comment ?? "",
    },
    mode: "onChange",
  });
  // updateExpert Trader
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    const id = selectedBot?._id;

    await updateTradingBot(id, userData);

    // form.reset(); // clear input
  }

  // update TradingBot Photo
  const savePhoto = () => {
    if (!profileImage) {
    ...
    updateTradingBotPhoto(id, formData);
  };
  ...
  return (
    <form id="form-rhf-demo" onSubmit={form.handleSubmit(onSubmit)}>
      <Card>
          ...
          <div className="flex items-center gap-3">
            <div className="relative w-28 min-w-28 h-28">
              <Image
                src={
                  imagePreview ??
                  (selectedBot?.photo && selectedBot.photo !== "No Photo"
                    ? selectedBot.photo
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Click the camera icon to change the Bot Icon
              </p>
            </div>
          </div>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  <Field data-invalid={fieldState.invalid}>
                    <FieldLabel htmlFor={field.name}>Bot Name</FieldLabel>
                    <InputGroup>
                      <InputGroupInput
                        {...field}
                        id={field.name}
        ...
        <CardFooter>
          <Button
            disabled={isUpdatingTradingBot}
            className="w-full"
            type="submit"
          >
            {isUpdatingTradingBot && <Spinner />}
            Update Trading Bot
          </Button>
        </CardFooter>
      </Card>
