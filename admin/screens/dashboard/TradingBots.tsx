...
import AddTradingBot from "@/components/AddTradingBot";
import UpdateTradingBot from "@/components/UpdateTradingBot";
...
const TradingBots = () => {
  ...
  const {
    ...
    isRefetching,
    deleteTradingBot,
    isDeletingTradingBot,
  } = useTradingBots();

  // console.log(allTradingBots);

  const [openDelete, setOpenDelete] = useState(false);

  const { openAddBot, setOpenAddBot, openEditBot, setOpenEditBot } =
    useSheetStore();

  const [selectedBot, setSelectedBot] = useState<TradingBotType | null>(null);

  const [tradingBotsList, setTradingBotsList] = useState<TradingBotType[]>([]);
  const [searchTerm, setSearchTerm] = useState("");

  const [page, setPage] = useState(0);
  const [rowsPerPage] = useState(12);

  const filteredTradingbots = Array.isArray(tradingBotsList)
  ...
  useEffect(() => {
    setTimeout(() => {
      if (allTradingBots.length !== 0) {
        setTradingBotsList(allTradingBots);
      }
    }, 0);
  }, [allTradingBots]);

  const paginatedbot = filteredTradingbots.slice(
    page * rowsPerPage,
    page * rowsPerPage + rowsPerPage
  );

  // delete Wallet Address
  const deleteBotrFunc = async () => {
    await deleteTradingBot(selectedBot?._id);
  ...
  if (pageLoading || isLoading || isDeletingTradingBot) {
    return (
    ...
    <div className="w-full space-y-4">
      ...
      <div className="w-full overflow-x-auto rounded-md border">
        <Table>
          ...
          <TableBody>
            {allTradingBots &&
              allTradingBots.length > 0 &&
              paginatedbot.map((bot) => (
                <TableRow
                  key={bot._id}
                  ...
                  <TableCell className="hidden xl:table-cell">
                    <div className="flex items-center gap-1 text-base">
                      {Intl.NumberFormat("en-US", {
                        style: "currency",
                        currency: "USD",
                        ...(bot?.price > 999999 ? { notation: "compact" } : {}),
                      }).format(bot?.price)}
                    </div>
                  </TableCell>
                  <TableCell className="hidden xl:table-cell">
      ...
      <SmartPagination
        page={page}
        totalItems={filteredTradingbots.length}
        rowsPerPage={rowsPerPage}
        onPageChange={setPage}
      />

      {/* ADD BOT  */}
      <Sheet open={openAddBot} onOpenChange={setOpenAddBot}>
        <SheetContent className="w-full! max-w-lg! data-[state=closed]:duration-300 data-[state=open]:duration-300">
          <SheetHeader className="border-b">
            <SheetTitle className="">Add Trading Bot</SheetTitle>
          </SheetHeader>
      ...
      {/* EDIT BOT  */}
      <Sheet open={openEditBot} onOpenChange={setOpenEditBot}>
        <SheetContent className="w-full! max-w-lg! data-[state=closed]:duration-300 data-[state=open]:duration-300">
          <SheetHeader className="border-b">
            <SheetTitle className="">Update Trading Bot</SheetTitle>
          </SheetHeader>

          <ScrollArea className="h-full overflow-y-auto">
            <div className="mx-4">
              <UpdateTradingBot selectedBot={selectedBot} />
            </div>
          </ScrollArea>
      ...
      <Dialog open={openDelete} onOpenChange={setOpenDelete}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Delete This bot {selectedBot?.name}</DialogTitle>
            <p className="text-sm text-muted-foreground mt-1">
              This action cannot be undone. Are you sure you want to permanently
              delete all this bot&apos;s data?
            </p>
          </DialogHeader>
