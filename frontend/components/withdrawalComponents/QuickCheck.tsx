...
const QuickCheck = ({
  ...
  const { withdrawFund, isWithdrawingFund } =
    useWithdrawalHistory(setSelectedView);
  ...
  const handleSubmit = async () => {
    try {
      if (user && user?.balance < quickCheckAmount) {
        ...
        <div className="rounded-lg bg-muted/50 p-4 space-y-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold">
              {Wallet === "Bank" ? "Bank" : walletAddress?.walletName}{" "}
              Withdrawal
            </h3>
          </div>
          ...
          <Button
            ...
            disabled={isWithdrawingFund}
            onClick={handleSubmit}
          >
            {isWithdrawingFund && <Spinner />}
            {isWithdrawingFund ? "Processing..." : "Initiate Withdrawal"}
          </Button>
        </div>
      </div>
    </>
