...
const TradingBots = () => {
  ...
  const { allTradingBots, isLoading, error, refetch, isRefetching } =
    useTradingBots();
  ...
  const [openViewbot, setOpenViewbot] = useState(false);

  const [botList, setbotList] = useState<TradingBotType[]>(allTradingBots);

  const [searchTerm, setSearchTerm] = useState("");

  const filteredAllBots = Array.isArray(botList)
    ? botList.filter(
        (tradingbot) =>
          tradingbot.name
            .toLowerCase()
    ...
    <Card className="relative">
      ...
      <CardContent className="grid gap-6 px-4 sm:px-6">
        <div className="grid grid-cols-2 sm:grid-cols-3 2xl:grid-cols-4 gap-4 sm:gap-6">
          {allTradingBots && allTradingBots.length > 0 ? (
            filteredAllBots.map((bot: TradingBotType, index: number) => (
              <div key={bot?._id}>
      ...
      {/* Screenshot Dialog */}
      <Dialog open={openViewbot} onOpenChange={setOpenViewbot}>
        <DialogContent className="sm:max-w-112.5 max-h-[90%] overflow-auto p-0 bg-secondary">
          ...
          <div className="relative w-full h-full p-2">
            <div className="relative">
              <Image
                src={selectedbot?.photo || "/qrCode_placeholder.jpg"}
                alt={selectedbot?.name || "image"}
                width={200}
                height={200}
              ...
              <div className="absolute bottom-0 left-0 bg-black/70 px-2 rounded-tr-xl">
                <p className="text-2xl font-semibold"> {selectedbot?.name}</p>
              </div>
            </div>
            ...
            <div className="px-1 mt-2">
              ...
              <div className="flex gap-1 text-base sm:text-xl text-green-400">
                ...
                <p className="font-bold">
                  ...
                  {Intl.NumberFormat("en-US", {
                    style: "currency",
                    currency: user?.currency?.code || "USD",
                    ...((selectedbot?.price || 0) > 99999
                      ? { notation: "compact" }
                      : {}),
                  }).format(selectedbot?.price || 0)}{" "}
                </p>
              </div>
              ...
              <div className="flex gap-1 mt-1">
                <p className="font-bold">Win rate:</p>
                <p>{selectedbot?.winRate}</p>
              </div>

              {/* Profit Share */}
              <div className="flex gap-1 mt-1 mb-2">
                <p className="font-bold"> Daily Trades:</p>
                <p>{selectedbot?.dailyTrades}</p>
              </div>

              <Separator />

              <div className="flex gap-1 mt-2">
                <p className="font-bold">Info: {selectedbot?.comment}</p>
              </div>
            </div>
          </div>
        </DialogContent>
