...
export default function Mailbox() {
  ...
  const { allUserMail, isLoading, refetch, isRefetching } = useGetUserMails();

  // console.log(allUserMail);

  // allMailInbox Messgaes
  const allMailInbox = useMemo(() => {
    if (!Array.isArray(allUserMail)) return [];

    const allMessages = allUserMail.reduce((accumulator, mailbox) => {
      const messagesWithUserId = mailbox.messages
        .filter((message: MailboxType) => message.from === "Support Team")
        .map((message: MailboxType) => ({
          ...message,
    ...
    // Sort newest → oldest
    allMessages.sort(
      (a: MailboxType, b: MailboxType) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    );
    return allMessages;
  }, [allUserMail]);

  // allMailSent Messages
  const allMailSent = useMemo(() => {
    if (!Array.isArray(allUserMail)) return [];

    const allMessages = allUserMail.reduce((accumulator, mailbox) => {
      const messagesWithUserId = mailbox.messages
        .filter((message: MailboxType) => message.from === user?.email)
        .map((message: MailboxType) => ({
          ...message,
          userId: mailbox.userId,
        }));
    ...
    return allMessages;
  }, [allUserMail, user?.email]);

  // allMailStarred Messages
  const allMailStarred = useMemo(() => {
    if (!Array.isArray(allUserMail)) return [];

    const allMessages = allUserMail.reduce((accumulator, mailbox) => {
      const messagesWithUserId = mailbox.messages
        .filter((message: MailboxType) => message.isUserStarred === true)
        .map((message: MailboxType) => ({
          ...message,
          userId: mailbox.userId,
        }));
  ...
  const handleStarredMail = (email: MailboxType) => {
    ...
    const messageData = {
      ...
      from: `userInboxComponent`, //which component ? sentComponent or inboxComponent etc
    };
  ...
  return (
    <div className="flex-1 sm:p-1 rounded-md">
      ...
      <div className="mb-4 mt-4 mx-2 sm:mx-5 px-4 py-2 bg-secondary rounded-md shadow-sm">
        <h1 className="font-semibold">Mailbox</h1>
      </div>

      <div className="h-[80vh] xs:h-[82vh] sm:h-[80vh] rounded-md bg-card shadow-md border border-slate-200 dark:border-muted overflow-hidden flex relative mt-3 mx-2 sm:mx-5">
        {/* LEFT SIDEBAR */}
        <aside className="hidden xl:flex flex-col gap-4 w-65 border-r border-slate-100 dark:border-muted p-4">
          <SidebarContent
        ...
        {/* MIDDLE: Email list */}
        <section className="w-full xl:w-87.5 2xl:w-105 border-r-0 xl:border-r border-slate-100 dark:border-muted flex flex-col">
          {/* Top header */}
