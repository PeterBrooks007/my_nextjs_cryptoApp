...
const addTradingBotsValidator = [
    ...
    body('userData.name')
      ...
      .isString().withMessage("Bot name must be a string")
    ...
    //   .isAlphanumeric().withMessage('First name must contain only letters and numbers and no white space.')
      .isLength({ max: 50 }).withMessage(' Bot name cannot exceed 50 characters')
      ...
      .custom(value => {
        ...
        return value.trim() === '' ? 
          Promise.reject(new Error(' Bot name cannot be empty or contain HTML entities')) : value;
      }),
    body('userData.price')
    ...
    .custom(value => {
        ...
        return value.trim() === '' ? 
          Promise.reject(new Error('dailyTradest cannot be empty or contain HTML entities')) : value;
      }),
...
const updateTradingBotsValidator = [
...
module.exports = { addTradingBotsValidator, updateTradingBotsValidator };
