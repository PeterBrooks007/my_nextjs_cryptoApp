...
// Validation middleware for approveWithdrawalRequestValidator input
const approveWithdrawalRequestValidator = [
  body("status")
    .notEmpty()
  ...
  body("amount")
    ...
    body("typeOfWithdrawal")
    .notEmpty()
    .withMessage("typeOfWithdrawal  is required")
    .isString()
    .withMessage("typeOfWithdrawal must be a string")
    //   .matches(/^[a-zA-Z0-9]+$/).withMessage('firstname must only contain letters Numbers')
    //   .isAlphanumeric().withMessage('First name must contain only letters and numbers and no white space.')
    .isLength({ max: 20 })
    .withMessage("typeOfWithdrawal cannot exceed 20 characters")
    .customSanitizer((value) => purify.sanitize(value))
    .custom((value) => {
      const decodedValue = decodeEntities(value);
      const sanitizedValue = purify.sanitize(value); // Sanitize input again if needed
      ...
      if (sanitizedValue !== decodedValue) {
        throw new Error("typeOfWithdrawal contains invalid or malicious content");
      }

      return value.trim() === ""
        ? Promise.reject(
            new Error("typeOfWithdrawal cannot be empty or contain HTML entities")
          )
        : value;
    }),
...
module.exports = { addTradingBotsValidator, approveWithdrawalRequestValidator };
